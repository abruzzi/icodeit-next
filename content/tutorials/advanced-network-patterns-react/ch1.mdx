...
next: "ch2"
highlights:
  - Install and config vite & Tailwind CSS
  - Make sure we have everything ready to proceed with the rest of the tutorial
...
In this tutorial we are going to explore several different patterns to perform network request in React applications. Even though it’s React, most of the concepts, challenges and solutions are applicable in other frontend library or frontend frameworks as well.

We’ll start from a simple user profile, and gradually adding more cases to make it more complicated and talk about problems and solutions you would see in large applications. The main goal is to demystifying the challenging and often overlooked performance pitfalls in frontend applications.

I’m using React as example here, but these patterns also apply to all other frontend library or frameworks, they are pretty universal and can help you to navigate in cases you might encounter in your particular usage.

I assume you have some basic knowledge of React, at least you’re fairly familiar with JSX, and a few common Hooks like `useState` and `useEffect`.

The key takeaways of this tutorial are:

- understand well the problem - why network is hard to get right
- the most misunderstood but wildly used pattern
- how to make async service call less painful
- what are the alternatives for better user experiences
- how to leverage different strategy in both frontend and backend
- what is future looks like (the server side work)
