...
const PostCard = ({ post }: { post: Post }) => {
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="font-heading my-0 inline-flex max-w-full flex-wrap items-baseline gap-x-2 text-xl font-semibold tracking-tight text-brand transition-colors duration-200 group-hover:text-slate-900 dark:group-hover:text-white sm:text-2xl">
          <span>{post.title}</span>
          <HiArrowRight
            className="inline-block size-5 shrink-0 translate-y-0.5 text-brand/70 transition-all duration-200 group-hover:translate-x-0.5 group-hover:text-slate-900 dark:group-hover:text-white sm:size-6"
            aria-hidden
          />
        </h2>
      </Link>
